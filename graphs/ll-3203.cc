...
public:
  ...
  void helper(vector<vector<int>> &adj, int node, int distance, int &farn,
              vector<int> &dis) {
    dis[node] = distance;
  ...
  int minimumDiameterAfterMerge(vector<vector<int>> &nums1,
    ...
    return max(
        {(diameter1 + 1) / 2 + (diameter2 + 1) / 2 + 1, diameter1, diameter2});
  }
};